...
public class TrialGeneratorASAP
{
    // Mode modifiers
    ...
    public TrialGeneratorASAP(Dataset datatset)
    {
        dataset = datatset;
        stimuli = new List<Stimulus>(datatset.stimuli);
        Debug.Log("Number of target number of stimulis in TrialGeneratorASAP: " + dataset.stimuli.Count);
    ...
    public virtual void Update()
    {
        ...
        {
            if (HasAnswerKeyBeenPressed() && (GetTrialDuration() <= minimumTrialDuration && !Input.GetKeyDown(KeyCode.LeftShift)))
    ...
    protected void PopulateTrials()
    {
        ...
        // if (shuffle)
        //     trialIndices.Shuffle(); // set to no because of ASAP
    }
    ...
    private void CreateTrialsFromScratch()
    {
        ...
        Debug.Log("new trials count from scratch: " + trialIndices.Count);

        // nAllTrials = dataset.stimuli.Count;
        nAllTrials = dataset.stimuli[0].batch;
    }
    ...
    private void ContinueTrialsSavedToFile()
    {
        ...
        currentTrialNumber = trials.Count;

        Debug.Log("TrialCollection loaded. Number of stimulis viewed: " + trials.Count);

        trialOffset = trials.Count; // all the trials listed in result file should be viewer/scored

        Debug.Log("Number of trials to finish: " + (nAllTrials - trialOffset));
        if (trialOffset == nAllTrials)
        {
            Debug.Log("trialOffset == nAllTrials -> NextScene");
            StaticMethods.NextScene();
        }
        else
        {
            ...
            foreach (Stimulus stimulus in dataset.stimuli)
            {
                Debug.Log("list stimulus.index "+stimulus.index);
                if(stimulus.index == trialOffset)
                {
                    Debug.Log("stimulus.index == trialOffset let continue with new stimuli"+stimulus.index);
            ...
            trialIndices = new List<int>();
            trialIndices.AddRange(System.Linq.Enumerable.Range(0, trials.Count));
            Debug.Log("number of new trials for comparison: " + (trialIndices.Count-trialOffset));
    ...
    {
        // TextAsset datasetFile = SessionData.GetAssetStimuliListFile();
        string datasetFile = SessionData.GetASAPStimuliListFile();
        ...
        Debug.Log("Number of stimulis generated now: " + dataset.stimuli.Count);
    }
    ...
    protected virtual void SaveResults()
    {
        ...
        Debug.Log("resultsDirectory " + resultsDirectory);
    ...
    public virtual bool NextTrial(bool iterate=true, int iterator = 1)
    {
        ...
        if(iterate)
        {
            ...
            Debug.Log("currentTrialNumber is " + currentTrialNumber);
            DestroyAllCurrentGameObjects();
        }
        Debug.Log("Iterating and destryoing objects");

        Debug.Log("currentTrialNumber is " + currentTrialNumber);
        Debug.Log("remaining stimuli in total " + (nAllTrials - currentTrialNumber));
        Debug.Log("remaining loaded stimuli " + (trialIndices.Count - currentTrialNumber));
        // if (currentTrialNumber < trialIndices.Count)
        if (currentTrialNumber < (nAllTrials))
        {
            Debug.Log("isNextTrialAvailable = true");
            isNextTrialAvailable = true;

            // Blank screen for waiting matlab updating stimuli file
            DisplayBlankScreen();

            do{
                System.Threading.Thread.Sleep(500);
                Debug.Log("Let's load the updated stimuli file!");
                ReadDataset();
                ContinueTrialsSavedToFile();
            }
            while (currentTrialNumber >= trialIndices.Count);

            GenerateTrial();
        }
        else
        {
            Debug.Log("isNextTrialAvailable = false");
    ...
    public virtual void GenerateTrial()
    {
        ...
        Debug.Log("length of trialIndices for GenerateTrial " + trialIndices.Count);
        Debug.Log("currentTrialNumber for GenerateTrial " + currentTrialNumber);
        currentTrialIndex = trialIndices[currentTrialNumber];
        currentTrialStartingTime = Time.time + blankScreenTime;

        Debug.Log("currentTrialIndex " + currentTrialIndex);
    ...
    public virtual void UpdateCurrentTime()
    {
        ...
        trials[currentTrialIndex].rotateLeft = rotationStartAlternate;
    }
    ...
    {
        return new Vector2Int(currentTrialNumber + 1, nAllTrials);
    }
    ...
    // IEnumerator WaitMatlab(float seconds)
    // {
